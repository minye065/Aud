...
void encode(float* passedInPCM, unsigned int totalSize, unsigned int sampleRate)
{
	...
	float* frameStorage = malloc(amountOfFrames * frameSize * sizeof(int16_t));
	...
	if (frameStorage && windowedFrameStorage)
	{
		...
		if (outFile)
		{
			fprintf(outFile, "SONG %u %u %d\n", sampleRate, hopSize, groupCounter);

			for (int i = 0; i < groupCounter; i++)
			{
				noteEvent thisNoteEvent = noteEventStorage[i];
				int startFrame = 0;
				if (i == 0)
				{
					startFrame = noteEventStorage[i].startFrame;
				}
				else
				{
					startFrame = noteEventStorage[i].startFrame - noteEventStorage[i - 1].startFrame;
				}
				fprintf(outFile, "NOTE %f %d %d %d\n", thisNoteEvent.fundamental, startFrame, thisNoteEvent.endFrame - thisNoteEvent.startFrame, thisNoteEvent.envelopeLength);
				float* thisEnvelope = thisNoteEvent.envelope;
				float value = 0;
				for (int ii = 0; ii < thisNoteEvent.envelopeLength; ii++)
				{
					if (ii == 0)
					{
						value = thisEnvelope[ii];
					}
					else
					{
						value = thisEnvelope[ii] - thisEnvelope[ii - 1];
					}
					fprintf(outFile, ii < thisNoteEvent.envelopeLength - 1 ? "%f," : "%f\n", value);
				}
			}
			fclose(outFile);
